...
- Obsidian's own scroll/key handling is kept out of the gestures at the source: the wheel listener is `{ passive: false }` and calls `preventDefault`; the pan-starting middle-button `mousedown` calls `preventDefault` (no autoscroll); and `Space` is handled on a window listener but only while the pointer is over the canvas, where it `preventDefault`s so it never swallows the space bar elsewhere in the app.
