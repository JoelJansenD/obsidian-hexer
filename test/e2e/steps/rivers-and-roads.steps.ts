...
Given('I am editing a river', async function (this: TestContext) {
    const rivers = await pathPage.getRivers();
    this.selectedRiver = rivers.find(river => river.id === EXISTING_RIVER_ID);
    expect(this.selectedRiver).toBeDefined();
...
When('I click on a hex', async function (this: TestContext) {
...
Then('the hex is added to the river', async function (this: TestContext) {
    expect(this.selectedRiver).toBeDefined();
...
Then('no edge is added', async function (this: TestContext) {
    expect(this.selectedRiver).toBeDefined();
    const river = await pathPage.getRiver(this.selectedRiver!.id);
    expect(river).toBeDefined();
